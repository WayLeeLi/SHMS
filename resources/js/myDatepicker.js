 ...
 *      endDate: 結束日期 默認是當前日期
 *      view:0 1 2 3 4 表示 "年月日周季"五個視圖
 *      parent: query selector； 選擇器添加到哪一個dom下面，默認是'body';
...
;(function($){
    $.fn.myDatePicker=function(inOptions){
        ...
        $(this).each(function(){
            let options={
                view:2,//0 1 2 3 4 5 6 7 表示 年 月 日 周 季 時 分 秒
                startDate:'1900-01-01 00:00:00',
                endDate:(new Date(2050,1,1)).Format("yyyy-mm-dd hh:ii:ss"),
            ...
            //這裡採用eval是為了方便動態去判斷parent;如果parent要動態的或者傳遞jquery物件可以把注釋打開；
            ...
            let dateTitles=['年','月','日','時','分','秒'];

            // process
            initDom();
            fixDate();//保留部分日期（格式合法 且 在區間 內）
            initSwiperAxis();
            bindListen();
            // api
            ...
            */
            function fixDate(){
                ...
                let times=[];
                let weeks=['周日','週一','週二','週三','週四','週五','週六'];
                for(var i in fixFunctions[options['view']]){
            ...
            function fixDateLimit(){
                ...
                switch(options['view']){
                    case 3:
                        //周 判斷endDate所在月是否有週一沒有則endDate前移一個月
                        ...
                        break;
                    case 4:
                        //季度 將startDate endDate改變成他們所在的季度的開始的那一個月
            ...
            function fixDay(){
                ...
                _dayUl.children('li').each(function(){
                    ...
                    //week 和 season week 只顯示每週 的週一 季度只顯示該月第一天
                    switch(options['view']){
                        case 3:
                            //week
            ...
            function checkMonth(date){
                let startDate=new Date(minYear,minMonth-1,1);//取當月第一天
                let endDate=new Date(maxYear,maxMonth-1,1);//取當月第一天
...
String.prototype.toDate = function (fmt){
    ...
    //safari不相容 2019-10-10 只能2019/10/10
