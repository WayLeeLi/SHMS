...
$(function() {
    ...
    // 单个图片删除
    $(".content-img-list").on("click", '.content-img-list-item a .icon-shanchu', function() {
    ...
    $(".content-img-list").on("click", '.content-img-list-item a .icon-iczoomout', function() {
        var index = $(this).parent().parent().parent().index();
        $(".modal-content").html("");
alert(imgSrc[index])
...
$('#upload').on('change', function(e) {
    ...
    if (imgSize > 1024 * 10240 * 1) { //1M
        return alert("上傳圖片不可大于10M");
    };
...
//提交请求
$('#dialogFormSubmit').on('click', function() {
    // FormData上传图片
...
function addNewContent(obj) {
    ...
    for (var a = 0; a < imgSrc.length; a++) {
        ...
        $(obj).html(oldBox + '<li class="content-img-list-item"><img src="' + imgSrc[a] + '" alt="">' +
            '<div class="hide"><a index="' + a + '" class="delete-btn"><i class="iconfont icon-shanchu"></i></a>' +
